...
function load_alumni() {
    ...
    let alumni = [];
    for (let i = 1; i < csvLines.length; i++) {
        const line = csvLines[i].trim(); // Trim whitespace
        if (line.length === 0) continue; // Skip empty lines

        const data = line.split(',');
        if (data.length < 4) continue; // Ensure there are enough columns
    ...
    // Sort alumni by year (numeric sorting for proper order)
    alumni.sort((a, b) => parseInt(b.year) - parseInt(a.year));

    // Clear containers
    let alumni_container = $("#alumni-section").empty();

    // Populate alumni cards
    for (let alumn of alumni) {
        let card = document.createElement('div');
        $(card).appendTo(alumni_container);
    
        // Check if 'page' URL is available
        if (alumn.page) {
            $(card).html(`
            <a href="${alumn.page}" target='_blank'>
                <p style="font-size: 0.5rem; font-weight: 300 !important; padding: .1rem .3rem; margin: 0 !important; text-align: left;">
                    <u>${alumn.name}</u>  (${alumn.year} - ${alumn.title})
                </p>
            </a>`);
        } else {
            $(card).html(`
            <p style="font-size: 0.5rem; font-weight: 300 !important; padding: .1rem .3rem; margin: 0 !important; text-align: left; color: black;">
                ${alumn.name}  (${alumn.year} - ${alumn.title})
            </p>`);
        }
    }
}
    ...
    for (const year of sortedYears) {
        ...
        for (const conference in groupedPublications[year]) {
            ...
            groupedPublications[year][conference].forEach(pub => {
                if (typeof pub !== 'undefined' && pub !== null && pub.authors) { // Check if pub and pub.authors are valid
                    ...
                    // Ensure pub.authors is a string before calling split()
                    const authors = typeof pub.authors === 'string' ? pub.authors.split(':').join(',') : 'No authors listed';
                    
                    $('<div>').addClass('icon').html(linkIcon).appendTo(entry_card);
                    $('<div>').addClass('details').html(pubTitle + '<br>' + authors).appendTo(entry_card);
                } else {
                    console.warn('Publication or authors is undefined:', pub);
                }
            });
        }
    }
